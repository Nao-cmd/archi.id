...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($location) || $price <= 0) {
        ...
        // Update database
        $update_query = "UPDATE projects SET title = ?, description = ?, location = ?, price = ?, image_thumbnail = ?, updated_at = NOW() WHERE id = ? AND user_id = ?";
        $update_stmt = $conn->prepare($update_query);
        $update_stmt->bind_param('sssisii', $title, $description, $location, $price, $image_thumbnail, $project_id, $user_id);
            ...
            <form method="POST" enctype="multipart/form-data" class="edit-form">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="price" name="price" required value="<?php echo $project['price']; ?>" min="0">
                    </div>
                </div>
